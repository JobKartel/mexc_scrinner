...
func (p *ProcessingEvent) computeMetrics(symbol string, trades []TradeData) *PairMetrics {
	...
	// фильтруем окно и игнорируем будущие метки
	filtered := make([]TradeData, 0, len(trades))
	for _, tr := range trades {
		if tr.TradeTime >= from && tr.TradeTime <= nowMs {
	...
	// гарантируем корректный порядок open/close
	sort.Slice(filtered, func(i, j int) bool { return filtered[i].TradeTime < filtered[j].TradeTime })
	...
	for _, tr := range filtered {
		...
		vol += tr.Price * tr.Quantity // для USDT/USDC это ~$ объём
	}

	// ВАЖНО: направленное (signed) изменение, а не диапазон
	change := 0.0
	if open > 0 {
		change = (closep - open) / open * 100.0
	}

	return &PairMetrics{
		Symbol:             symbol,
		PriceChangePercent: change, // теперь может быть и отрицательным
		TotalVolume:        vol,
		OpenPrice:          open,
		ClosePrice:         closep,
	}
